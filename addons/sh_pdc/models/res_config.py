...
class ResConfigSettings(models.TransientModel):
    ...
    pdc_payment_approval = fields.Boolean('Customer Payment Approval', config_parameter='sh_pdc.pdc_payment_approval')
    pdc_approval_amount = fields.Float('Minimum Approval Amount', config_parameter='sh_pdc.pdc_approval_amount')
    pdc_approval_currency_id = fields.Many2one('res.currency', string='Approval Currency',
                                           config_parameter='sh_pdc.pdc_approval_currency_id',
                                           help="Converts the payment amount to this currency if chosen.")
    pdc_vendor_payment_approval = fields.Boolean('Vendor Payment Approval', config_parameter='sh_pdc.pdc_vendor_payment_approval')
    pdc_vendor_approval_amount = fields.Float('Minimum Approval Amount', config_parameter='sh_pdc.pdc_vendor_approval_amount')
    pdc_vendor_approval_currency_id = fields.Many2one('res.currency', string='Approval Currency',
                                           config_parameter='sh_pdc.pdc_vendor_approval_currency_id',
                                           help="Converts the payment amount to this currency if chosen.")
