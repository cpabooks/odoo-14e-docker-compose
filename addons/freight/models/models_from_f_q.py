...
class QuotationInherit(models.Model):
    ...
    direction = fields.Selection(([('import', 'Import'), ('export', 'Export'), ('crosstrade', 'Crosstrade')]),
                                 string='Direction')
            ...
            for line in self.order_line:
                if line.selected:
                    ...
                    line_vals = {
                        ...
                        # "rate_in_foreign_currency": line.rate_in_foreign_currency,
                    ...
                    if line.product_id.charge_type.id == self.env.ref(
                            'freight.freight_charge_type_main').id:
                        for val in containerSizes:
                            line_vals['container_size'] = val.id
                            if val.id == 1:
                                line_vals['rate_in_foreign_currency'] = line['freight_20ge']
                                line_vals['price_unit'] = line['freight_20ge']*line.ex_currency_rate
                            elif val.id == 2:
                                line_vals['rate_in_foreign_currency'] = line['freight_40ge']
                                line_vals['price_unit'] = line['freight_40ge'] * line.ex_currency_rate
                            elif val.id == 3:
                                line_vals['rate_in_foreign_currency'] = line['freight_40hc']
                                line_vals['price_unit'] = line['freight_40hc'] * line.ex_currency_rate
...
class freightInvoicecontainers(models.Model):
    ...
    def container_qty_changed(self):
        invoiceLines = self.env['account.move.line'].search(
            [("move_id", '=', self.move_id._origin.id), ("freight_container_size", '=', self.container_size.id),
...
class QuotationLineFrFieldInherit(models.Model):
    ...
    def _get_default_pol_id(self):
        return self.env['freight.port'].search([('name', '=', 'Jabel Ali')], limit=1)

    def _get_default_pod_id(self):
        return self.env['freight.port'].search([('name', '=', 'Port Khalid')], limit=1)

    def _get_default_container_type(self):
        return self.env['freight.container.type'].search([('name', '=', 'Open Top')], limit=1)
    ...
    port_of_loading = fields.Many2one('freight.port', string="POL", default=_get_default_pol_id)
    port_of_delivery = fields.Many2one('freight.port', string="POD", default=_get_default_pod_id)
    freight_item_qty = fields.Integer("Vol. Qty", store=True)
    freight_container_type = fields.Many2one('freight.container.type', string="Container Type", default=_get_default_container_type)
    rate_basis = fields.Many2one("freight.rate.basis", "Rate Basis")
    ...
    freight_20ge = fields.Float(string="20 GE", digits=(12, 1),
                                store=True)  # compute="get_container_freight", inverse="set_container_freight")
    ...
    def open_to_form_view(self, context=None):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': self.env.ref('freight.freight_order_line_form').id,
    ...
    def action_view_details(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "freight.freight_quotation_line_details_action")
    ...
    def get_product_price(self):
        for rec in self:
            rec.price_unit = rec.product_id.with_context(
                pricelist=self.sale_order_id.partner_id.property_product_pricelist.id).price
            rec.price_subtotal = rec.price_unit * rec.product_uom_qty
