...
class EmpPayslipReport(models.TransientModel):
    ...
    def employee_ctc_report(self):
        if self.file_type == 'pdf':
            ...
            # Merge Row Columns
            TITLEHEDER = 'Employee CTC Report'
            ...
            worksheet.write(4, 7, 'Leave Salary', sub_cell_wrap_format_bold)
            worksheet.write(4, 8, 'Gratuity', sub_cell_wrap_format_bold)
            worksheet.write(4, 9, 'Food Allow.', sub_cell_wrap_format_bold)
            worksheet.write(4, 10, 'Tell Allow.', sub_cell_wrap_format_bold)
            worksheet.write(4, 11, 'Vehicle Allow.', sub_cell_wrap_format_bold)
            worksheet.write(4, 12, 'Other Allow.', sub_cell_wrap_format_bold)
            worksheet.write(4, 13, 'Total Cont. Amount', sub_cell_wrap_format_bold)
            worksheet.write(4, 14, 'Rate Dedn (365)', sub_cell_wrap_format_bold)
            worksheet.write(4, 15, 'Rate', sub_cell_wrap_format_bold)

            worksheet.write(4, 16, 'NOT Rate', sub_cell_wrap_format_bold)
            worksheet.write(4, 17, 'HOT Rate.', sub_cell_wrap_format_bold)
            worksheet.write(4, 18, 'W/D (c/d)', sub_cell_wrap_format_bold)
            worksheet.write(4, 19, 'Hours', sub_cell_wrap_format_bold)

            worksheet.merge_range(3, 20, 3, 24, 'Attendance / TS', cell_wrap_format_bold)
            worksheet.write(4, 20, 'Basic Hour', sub_cell_wrap_format_bold)
            worksheet.write(4, 21, 'NOT Hour', sub_cell_wrap_format_bold)
            worksheet.write(4, 22, 'HOT Hour', sub_cell_wrap_format_bold)
            worksheet.write(4, 23, 'Absent Hour', sub_cell_wrap_format_bold)
            worksheet.write(4, 24, 'Total Hour.', sub_cell_wrap_format_bold)
            ...
            for payslip in payslip_ids:
                ...
                ###################################Work day calculation########################################
                get_emp_not_hour = sum(self.env['hr.overtime.line'].search(
                    [('date', '>=', payslip.date_from), ('date', '<=', payslip.date_to), ('ot_type', '=', 'not'),
                     ('employee_id', '=', payslip.employee_id.id)]).mapped('number_of_hours'))
                get_emp_hot_hour = sum(self.env['hr.overtime.line'].search(
                    [('date', '>=', payslip.date_from), ('date', '<=', payslip.date_to), ('ot_type', '=', 'hot'),
                     ('employee_id', '=', payslip.employee_id.id)]).mapped('number_of_hours'))
                ...
                if payslip.employee_id.contract_id.wage_type == 'monthly':
                    ...
                    values['Leave_sal'] = round((payslip.employee_id.contract_id.wage / 11), 2) or 0,
                    values['gratuity'] = round((payslip.employee_id.contract_id.wage / 12), 2) or 0,
                ...
                if payslip.employee_id.contract_id.wage_type == 'hourly':
                    ...
                    values['Leave_sal'] = round((payslip.employee_id.contract_id.hourly_wage / 11), 2) or 0,
                    values['gratuity'] = round((payslip.employee_id.contract_id.hourly_wage / 12), 2) or 0,
                ...
                values['Total_Alw'] = round(
                    payslip.employee_id.contract_id.wage + payslip.employee_id.contract_id.food_allowance + \
                    payslip.employee_id.contract_id.telephone_allowance + payslip.employee_id.contract_id.vehicle_allowance + payslip.employee_id.contract_id.other_allowance,
                    2) or 0,
                values['Rate'] = payslip.employee_id.contract_id.salary_per_h or 0,
                values['Rate'] = payslip.employee_id.contract_id.salary_per_h or 0,
                # values['Not_Rate'] = payslip.struct_id.not_rate  or 0,
                values['Not_Rate'] = payslip.contract_id.not_rate if (
                            payslip.contract_id.not_rate > 0 and payslip.contract_id.not_rate != payslip.struct_id.not_rate) else payslip.struct_id.not_rate,
                # values['Hot_Rate'] = payslip.struct_id.hot_rate or 0,
                values['Hot_Rate'] = payslip.contract_id.hot_rate if (
                            payslip.contract_id.hot_rate > 0 and payslip.contract_id.hot_rate != payslip.struct_id.hot_rate) else payslip.struct_id.hot_rate,
                if payslip.employee_id.contract_id.wage_type == 'monthly':
                ...
                if payslip.employee_id.contract_id.wage_type == 'hourly':
                    ...
                    values['Basic_hour'] = round(payslip.net_wage / payslip.employee_id.contract_id.hourly_wage,
                                                 2) or 0,
                values['Not_Hour'] = get_emp_not_hour or 0,
                values['Hot_Hour'] = get_emp_hot_hour or 0,
                values['Absent_Hour'] = total_absent_hour or 0,
                values['Total_Hour'] = round(basic_hour + get_emp_not_hour + get_emp_hot_hour + total_absent_hour,
                                             2) or 0,
                ...
                for i, j in lable.items():
                    if i not in ['no', 'payslip_ref', 'employee', 'designation', 'period', 'Payrules', 'Basic_Sal','Leave_sal','gratuity',
                                 'Food_Alw', 'Tell_Alw', 'Vehicle_Alw', 'Other_Alw', 'Total_Alw', 'Rate_Dedn', 'Rate',
                                 'Not_Rate', 'Hot_Rate', 'Work_Day', 'Hours', 'Basic_hour', 'Not_Hour', 'Hot_Hour',
                                 'Absent_Hour', 'Total_Hour']:
            ...
            for value in main:
                col = 0
                row = row
                list = ['NO', 'Payslip_Ref', 'Employee', 'Designation', 'Period', 'Payrules', 'Basic_Sal','Leave_sal','gratuity', 'Food_Alw',
                        'Tell_Alw', 'Vehicle_Alw', 'Other_Alw', 'Total_Alw', 'Rate_Dedn', 'Rate', 'Not_Rate',
                        'Hot_Rate', 'Work_Day', 'Hours', 'Basic_hour', 'Not_Hour', 'Hot_Hour', 'Absent_Hour',
            ...
            # For Get the Total
            total_row = end_row + 1
            list = ['Basic_Sal','Leave_sal','gratuity', 'Food_Alw', 'Tell_Alw', 'Vehicle_Alw', 'Other_Alw', 'Total_Alw', 'Rate_Dedn', 'Rate',
