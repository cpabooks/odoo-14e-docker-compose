...
class InvoiceTemplates(models.Model):
    ...
    show_contract_invoice = fields.Boolean('Show Contracting Invoice', default=False)
    total_work_done_sum = fields.Float(
        string="Total Work Done",
        compute="_compute_total_work_done_sum",
        store=True
    )
    invoice_type = fields.Many2one('invoice.type',string='Invoice Type', required=True)
    # tax_amt_sum = fields.Float(
    #     string="Total Tax (Work Done)",
    #     compute="_compute_tax_amt_sum",
    #     store=True
    # )
    ...
    def write(self, vals):
        line_ids = vals.get('invoice_line_ids', False)
        if line_ids:
            for line in line_ids:
                if isinstance(line, (list, tuple)) and len(line) >= 3:
                    values = line[2]
                    # Check if line[1] is a real ID (not virtual)
                    if values and line[1] and not isinstance(line[1], str):
                        # Only browse if it's a real integer ID
                        if isinstance(line[1], int):
                            self.env['account.move.line'].browse(line[1]).write(values)
        return super(InvoiceTemplates, self).write(vals)
    ...
    @api.depends('invoice_line_ids.total_work_done')
    def _compute_total_work_done_sum(self):
        for move in self:
            move.total_work_done_sum = sum(move.invoice_line_ids.mapped('total_work_done'))

    # @api.depends(
    #     'invoice_line_ids.total_work_done',
    #     'invoice_line_ids.price_total'
    # )
    # def _compute_amount(self):
    #     super(InvoiceTemplates, self)._compute_amount()
    #
    #     for move in self:
    #         if move.show_contract_invoice:
    #             move.amount_untaxed = move.total_work_done_sum
    #             # move.amount_total = move.amount_untaxed + move.amount_tax

    @api.depends(
        'invoice_line_ids.total_work_done',
        'invoice_line_ids.tax_ids',
        'show_contract_invoice'
    )
    def _compute_amount(self):
        for move in self:
            if move.show_contract_invoice:
                # Untaxed Amount = sum of total_work_done
                total_untaxed = move.total_work_done_sum

                # Tax Amount = sum of each line's (total_work_done * tax %)
                total_tax = 0.0
                for line in move.invoice_line_ids:
                    if line.tax_ids:
                        for tax in line.tax_ids:
                            total_tax += line.total_work_done * (tax.amount / 100)

                move.amount_untaxed = total_untaxed
                move.amount_tax = total_tax
                move.amount_total = total_untaxed + total_tax
                move.amount_residual = move.amount_total  # optional
                # Fix the Tax group display line (Tax 5%: X AED)
                tax_group_amounts = {}
                for line in move.invoice_line_ids:
                    for tax in line.tax_ids:
                        group = tax.tax_group_id
                        tax_amount = line.total_work_done * (tax.amount / 100)
                        if group not in tax_group_amounts:
                            tax_group_amounts[group] = {
                                'tax_group_name': group.name,
                                'tax_group_amount': 0.0,
                                'tax_group_base_amount': 0.0,
                                'tax_group_id': group.id,
                            }
                        tax_group_amounts[group]['tax_group_amount'] += tax_amount
                        tax_group_amounts[group]['tax_group_base_amount'] += line.total_work_done

                move.amount_by_group = [
                    (
                        vals['tax_group_name'],
                        vals['tax_group_amount'],
                        vals['tax_group_base_amount'],
                        # formatted strings (Odoo uses these for display)
                        f"{vals['tax_group_amount']:.2f}",
                        f"{vals['tax_group_base_amount']:.2f}",
                        vals['tax_group_id'],
                        len(tax_group_amounts),
                    )
                    for vals in tax_group_amounts.values()
                ]


            else:
                # Fall back to standard Odoo computation
                super(InvoiceTemplates, move)._compute_amount()
