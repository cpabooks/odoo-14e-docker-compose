...
class res_pertner(models.Model):
    ...
    balance_invoice_ids = fields.One2many('account.move', 'partner_id', 'Customer move lines', domain=[('move_type', 'in', ['out_invoice','out_refund','in_invoice','in_refund']),('payment_state', 'not in', ['paid']),('state','=','posted')]) 
...
class sale_order(models.Model):
    ...
    def _compute_total_receivable(self):
        for order in self:
            ...
            if order.partner_id.credit:
                order.sudo().update({'total_receivable' : self.partner_id.credit})
            if not order.partner_id.credit:
                order.sudo().update({'total_receivable' : self.partner_id.parent_id.credit})
    ...
    def onchange_partner_id(self):
        ...
        if not self.partner_id.credit:
            self.partner_id.sudo().credit = self.partner_id.parent_id.credit
