# -*- coding: utf-8 -*-
# Part of BrowseInfo. See LICENSE file for full copyright and licensing details.

{
	'name':"All in One Stock Transfer Backdate and Remarks in Odoo",
	'version':'13.0.0.7',
	'category':'Warehouse',
	'summary':'inventory adjustment backdate all stock transfer backdate transfer backdating stock backdate remarks inventory adjustment backdating remarks stock operation backdate receipt backdate all stock back-date shipment backdate receipt internal transfer backdate',
	'description':"""

	     odoo stock transfer backdating stock transfers backdating inventory stock transfer backdating
     odoo inventory transfer backdating inventory transfer backdating
     odoo stock backdate Stock Transfers Backdate inventory Transfers Backdate
     odoo inventory backdating option stock backdating options Inventory Backdate Operations
     odoo Backdate Options backdating options Inventory Backdate Operations Backdate Operations
     odoo warehouse backdate operations warehouse stock backdate operations
     odoo stock backdate odoo stock remarks transfer remarks warehouse delivery remarks warehouse remarks
     odoo inventory remarks odoo stock backdate remarks
     odoo stock accounting backdate transfer backdate on stock odoo


     odoo stock transfer back dating stock transfers back dating inventory stock transfer back dating
     odoo inventory transfer back dating inventory transfer back dating odoo
     odoo stock back date Stock Transfers Back-date inventory Transfers Back date
     odoo inventory back-dating option stock back dating options Inventory Back date Operations
     odoo Back-date Options back dating options Inventory Back date Operations Back date Operations
     odoo warehouse back date operations warehouse stock back date operations
     odoo stock back date odoo stock remarks transfer remarks warehouse delivery remarks warehouse remarks
     odoo inventory remarks odoo stock back date remarks
     odoo stock accounting back date transfer back date on inventory odoo
    
    
    
     entries so to avoid the problem this app will help to put custom back date and remarks.
     Custom back date will be transfer to stock entries and accounting entries

     You can set a scrap Backdate and Remarks in stock move product move and account move in one click.
    Odoo scrap order Backdate and Remarks
     odoo scrap transfer backdating stock scrap backdating scrap transfer backdating
     odoo scrap backdating scrap order transfer backdating
     odoo scrap backdate scrap order Backdate scrap order transfer Backdate
     odoo scrap backdating option scrap backdating options scrap Backdate Operations
     odoo scrap Backdate Options backdating options scrap Backdate Operations Backdate Operations
     odoo scrap backdate operations scrap stock backdate operations
     odoo scrap order backdate odoo scrap order remarks scrap transfer remarks warehouse scrap order remarks scrap remarks
     odoo scrap remarks odoo scrap backdate remarks
     odoo scrap accounting backdate transfer backdate on scrap odoo


     odoo scrap order back dating scrap order transfers back dating scrap order stock transfer back dating
     odoo scrap order transfer back dating inventory scrap order back dating odoo
     odoo scrap order back date scrap order Back-date scrap order Transfers Back date
     odoo scrap order back-dating option scrap order back dating options Inventory Back date Operations
     odoo Back-date Options back dating options Inventory Back date Operations Back date Operations
     odoo warehouse back date operations warehouse scrap order back date operations
     odoo scrap order back date odoo scrap order remarks transfer remarks warehouse scrap order remarks warehouse remarks
     odoo inventory remarks odoo scrap order back date remarks
     odoo scrap order accounting back date transfer back date on inventory odoo

     odoo stock transfer backdating stock transfers backdating inventory stock transfer backdating
     odoo inventory transfer backdating inventory transfer backdating
     odoo stock backdate Stock Transfers Backdate inventory Transfers Backdate
     odoo inventory backdating option stock backdating options Inventory Backdate Operations
     odoo Backdate Options backdating options Inventory Backdate Operations Backdate Operations
     odoo warehouse backdate operations warehouse stock backdate operations
     odoo stock backdate odoo stock remarks transfer remarks warehouse delivery remarks warehouse remarks
     odoo inventory remarks odoo stock backdate remarks
     odoo stock accounting backdate transfer backdate on stock odoo


     odoo stock transfer back dating stock transfers back dating inventory stock transfer back dating
     odoo inventory transfer back dating inventory transfer back dating odoo
     odoo stock back date Stock Transfers Back-date inventory Transfers Back date
     odoo inventory back-dating option stock back dating options Inventory Back date Operations
     odoo Back-date Options back dating options Inventory Back date Operations Back date Operations
     odoo warehouse back date operations warehouse stock back date operations
     odoo stock back date odoo stock remarks transfer remarks warehouse delivery remarks warehouse remarks
     odoo inventory remarks odoo stock back date remarks
     odoo stock accounting back date transfer back date on inventory odoo
    
	odoo internal transfer backdate internal transfer remarks internal transfer backdate and remarks,
	odoo receipt backdate receipt remarks receipt backdate and remarks,
	odoo internal transfer and receipt backdate internal transfer and receipt remarks,
	odoo internal transfer and receipt backdate and remarks,

     odoo stock transfer backdating stock transfers backdating inventory stock transfer backdating
     odoo inventory transfer backdating inventory transfer backdating
     odoo stock backdate Stock Transfers Backdate inventory Transfers Backdate
     odoo inventory backdating option stock backdating options Inventory Backdate Operations
     odoo Backdate Options backdating options Inventory Backdate Operations Backdate Operations
     odoo warehouse backdate operations warehouse stock backdate operations
     odoo stock backdate odoo stock remarks transfer remarks warehouse delivery remarks warehouse remarks
     odoo inventory remarks odoo stock backdate remarks
     odoo stock accounting backdate transfer backdate on stock odoo


     odoo stock transfer back dating stock transfers back dating inventory stock transfer back dating
     odoo inventory transfer back dating inventory transfer back dating odoo
     odoo stock back date Stock Transfers Back-date inventory Transfers Backdate
     odoo inventory back-dating option stock back dating options Inventory Back date Operations
     odoo Back-date Options back dating options Inventory Back date Operations Back date Operations
     odoo warehouse back date operations warehouse stock back date operations
     odoo stock back date odoo stock remarks transfer remarks warehouse delivery remarks warehouse remarks
     odoo inventory remarks odoo stock back date remarks
     odoo stock accounting back date transfer back date on inventory odoo

     odoo receipt back dating stock receipt back dating purchase receipt back dating
     odoo purchase receipt transfer back dating receipt back dating odoo
     odoo stock receipt back date receipts Back-date purchase receipt Back date
     odoo purchase receipt back-dating option receipt back dating options receipt Back date Operations
     odoo receipt Back-date Options receipt back dating options receipt Back date Operations Back date Operations receipt
     odoo receipt back date operations receipt stock back date operations
     odoo receipt back date odoo receipt remarks transfer remarks receipt delivery remarks receipt remarks
     odoo receipt remarks odoo stock receipt back date remarks
     odoo purchase receipt accounting back date transfer back date on receipt odoo

     odoo Internal Transfer back dating stock Internal Transfer back dating Internal Transfer back dating
     odoo stock Internal Transfer back dating Internal Transfer back dating odoo
     odoo stock Internal Transfer back date Internal Transfers Back-date purchase Internal Transfers Back date
     odoo Internal Transfer back-dating option Internal Transfer back dating options Internal Transfer Back date Operations
     odoo Internal Transfer Back-date Options Internal Transfer back dating options Internal Transfers Back date Operations Back date Operations Internal Transfer
     odoo Internal Transfer back date operations Internal Transfers stock back date operations
     odoo Internal Transfer back date odoo Internal Transfer remarks Internal Transfer remarks Internal Transfer delivery remarks Internal Transfer remarks
     odoo Internal Transfer remarks odoo stock Internal Transfer back date remarks
     odoo stock Internal Transfer accounting back date Internal Transfer back date on Internal Transfer odoo

     odoo delivery backdating stock delivery backdating delivery transfer backdating
     odoo delivery order backdating delivery transfer backdating
     odoo delivery order backdate delivery order Backdate delivery Transfers Backdate
     odoo delivery backdating option delivery backdating options delivery operation Backdate on delivery order
     odoo Backdate Options backdating options delivery order Backdate Operations Backdate Operations for delivery
     odoo warehouse delivery order backdate operations delivery order backdate operations
     odoo delivery backdate odoo delivery remarks delivery order remarks delivery remarks stock delivery remarks
     odoo delivery process remarks odoo delivery process backdate remarks
     odoo delivery accounting backdate delivery backdate on goods transfer odoo
    
     entries so to avoid the problem this app will help to put custom back date and remarks.
     Custom back date will be transfer to stock entries and accounting entries  



     odoo Inventory Adjustment backdating Inventory Adjustments backdating inventory backdating
     odoo inventory stock backdating inventory back date
     odoo Inventory Adjustment backdate Inventory Adjustment Transfers Backdate Inventory Adjustment Backdate
     odoo Inventory Adjustments backdating option Inventory Adjustments backdating options Inventory Adjustments Backdate Operations
     odoo Backdate Options backdating options Inventory Adjustments Backdate Operations Backdate Operations on Inventory Adjustments
     odoo warehouse backdate operations warehouse stock backdate operations
     odoo stock Inventory backdate odoo stock Inventory Adjustment remarks Inventory Adjustments remarks warehouseInventory Adjustments remarks
     odoo opening inventory remarks odoo Inventory Adjustment backdate remarks opening stock balance backdate
     odoo Inventory Adjustment accounting backdate Inventory Adjustments backdate on stock odoo


     odoo Inventory Adjustment back dating Inventory Adjustments back dating Inventory Adjustments back dating
     odoo Inventory Adjustment back dating Inventory Adjustments back dating odoo
     odoo Inventory Adjustments back date Inventory Adjustments Back-date Inventory Adjustment Back date
     odoo Inventory Adjustment back-dating option Inventory Adjustments back dating options Inventory Adjustment Back date Operations
     odoo Back-date Options back dating options Inventory Back date Operations Back date Operations
     odoo Inventory Adjustment back date operations Inventory Adjustments stock back date operations
     odoo Inventory Adjustment back date odoo Inventory Adjustment remarks transfer remarks Inventory Adjustments remarks warehouse remarks
     odoo Inventory Adjustment remarks odoo Inventory Adjustment back date remarks
     odoo Inventory Adjustment accounting back date Inventory Adjustment back date on inventory odoo  

 	Manufacturing order backdate manufacturing order backdate remarks workorder backdate remarks
	Odoo workorder remarks manufacturing backdate and remarks 

     odoo stock transfer backdating stock transfers backdating inventory stock transfer backdating
     odoo inventory transfer backdating inventory transfer backdating
     odoo stock backdate Stock Transfers Backdate inventory Transfers Backdate
     odoo inventory backdating option stock backdating options Inventory Backdate Operations
     odoo Backdate Options backdating options Inventory Backdate Operations Backdate Operations
     odoo warehouse backdate operations warehouse stock backdate operations
     odoo stock backdate odoo stock remarks transfer remarks warehouse delivery remarks warehouse remarks
     odoo inventory remarks odoo stock backdate remarks
     odoo stock accounting backdate transfer backdate on stock odoo


     odoo stock transfer back dating stock transfers back dating inventory stock transfer back dating
     odoo inventory transfer back dating inventory transfer back dating odoo
     odoo stock back date Stock Transfers Back-date inventory Transfers Back date
     odoo inventory back-dating option stock back dating options Inventory Back date Operations
     odoo Back-date Options back dating options Inventory Back date Operations Back date Operations
     odoo warehouse back date operations warehouse stock back date operations
     odoo stock back date odoo stock remarks transfer remarks warehouse delivery remarks warehouse remarks
     odoo inventory remarks odoo stock back date remarks
     odoo stock accounting back date transfer back date on inventory odoo
    
    
    
     entries so to avoid the problem this app will help to put custom back date and remarks.
     Custom back date will be transfer to stock entries and accounting entries


     odoo Manufacturing backdating Manufacturing backdating inventory Manufacturing backdating
     odoo Manufacturing backdating Manufacturing inventory backdating
     odoo Manufacturing backdate Manufacturing Backdate Manufacturing stock Transfer Backdate
     odoo Manufacturing backdating option Manufacturing backdating options Manufacturing Backdate Operations
     odoo Backdate Options on Manufacturing backdating options Manufacturing Backdate Operations Backdate Operations on Manufacturing
     odoo Manufacturing backdate operations Manufacturing stock backdate operations
     odoo Manufacturing backdate odoo Manufacturing remarks Manufacturing remarks Manufacturing orders remarks
     odoo Manufacturing remarks odoo Manufacturing backdate remarks
     odoo Manufacturing accounting backdate transfer backdate on Manufacturing odoo


     odoo Manufacturing transfer back dating Manufacturing transfers back dating Manufacturing transfer back dating
     odoo Manufacturing transfer back dating Manufacturing transfer back dating odoo
     odoo Manufacturing back date Manufacturing Transfers Back-date Manufacturing Transfers Back date
     odoo Manufacturing back-dating option Manufacturing back dating options Manufacturing Back date Operations
     odoo Back-date Options Manufacturing back dating options Manufacturing Back date Operations Back date Operations
     odoo Manufacturing back date operations Manufacturing Manufacturing back date operations
     odoo Manufacturing back date odoo Manufacturing remarks transfer remarks Manufacturing delivery remarks Manufacturing remarks
     odoo Manufacturing remarks odoo stock back date remarks
     odoo Manufacturing stock accounting back date transfer back date on Manufacturing odoo
    
    
     odoo Manufacturing order backdating Manufacturing order backdating inventory Manufacturing order backdating
     odoo Manufacturing order backdating Manufacturing order inventory backdating
     odoo Manufacturing order backdate Manufacturing order Backdate Manufacturing order stock Transfer Backdate
     odoo Manufacturing order backdating option Manufacturing order backdating options Manufacturing order Backdate Operations
     odoo Backdate Options on Manufacturing order backdating options Manufacturing order Backdate Operations Backdate Operations on Manufacturing order
     odoo Manufacturing order backdate operations Manufacturing order stock backdate operations
     odoo Manufacturing order backdate odoo Manufacturing order remarks Manufacturing order remarks Manufacturing order remarks
     odoo Manufacturing order remarks odoo Manufacturing orders backdate remarks
     odoo Manufacturing order accounting backdate transfer backdate on Manufacturing orders


     odoo Manufacturing order transfer back dating Manufacturing order transfers back dating Manufacturing order transfer back dating
     odoo Manufacturing order transfer back dating Manufacturing order transfer back dating odoo
     odoo Manufacturing order back date Manufacturing Transfers Back-date Manufacturing Transfers Back date
     odoo Manufacturing order back-dating option Manufacturing back dating options Manufacturing Back date Operations
     odoo Back-date Options Manufacturing order back dating options Manufacturing Back date Operations Back date Operations
     odoo Manufacturing order back date operations Manufacturing Manufacturing back date operations
     odoo Manufacturing order back date odoo Manufacturing remarks transfer remarks Manufacturing delivery remarks Manufacturing remarks
     odoo Manufacturing order remarks odoo stock back date remarks
     odoo Manufacturing order stock accounting back date manufacture back date on Manufacturing order odoo
    
     entries so to avoid the problem this app will help to put custom back date and remarks.
     Custom back date will be transfer to stock entries and accounting entries


     odoo Manufacture back dating Manufacture transfers back dating Manufacture transfer back dating
     odoo Manufacture transfer back dating Manufacture transfer back dating odoo
     odoo Manufacture back date Manufacture Transfers Back-date Manufacture Transfers Back date
     odoo Manufacture back-dating option Manufacture back dating options Manufacture Back date Operations
     odoo Back-date Options Manufacture back dating options Manufacture Back date Operations Back date Operations
     odoo Manufacture back date operations Manufacture back date operations
     odoo Manufacture back date odoo Manufacture remarks transfer remarks Manufacture delivery remarks Manufacture remarks
     odoo Manufacture remarks odoo stock back date remarks
     odoo Manufacture stock accounting back date transfer back date on Manufacture odoo



     odoo MRP back dating MRP transfers back dating MRP transfer back dating
     odoo MRP transfer back dating MRP transfer back dating odoo
     odoo MRP back date MRP Transfers Back-date MRP Transfers Back date
     odoo MRP back-dating option MRP back dating options MRP Back date Operations MRP
     odoo Back-date Options MRP back dating options MRP Back date Operations Back date Operations
     odoo MRP back date operations MRP back date operations
     odoo MRP back date odoo MRP remarks transfer remarks MRP delivery remarks MRP remarks
     odoo MRP remarks odoo stock back date remarks
     odoo MRP stock accounting back date transfer back date on MRP odoo


     odoo MO back dating MO transfers back dating MO transfer back dating
     odoo MO transfer back dating MO transfer back dating odoo
     odoo MO back date MO Transfers Back-date MO Transfers Back date
     odoo MO back-dating option MO back dating options MO Back date Operations MO
     odoo Back-date Options MO back dating options MO Back date Operations Back date Operations on MO
     odoo MO back date operations MO back date operation MO
     odoo MO back date odoo MO remarks transfer remarks MO delivery remarks MO remarks on MO
     odoo MO remarks odoo stock back date remarks on MO
     odoo MO stock accounting back date transfer back date on MO odoo

     odoo Workorder back dating Workorder transfers back dating Workorder transfer back dating on Workorder
     odoo Workorder transfer back dating Workorder transfer back dating Workorder odoo
     odoo Workorder back date Workorder Transfers Back-date Workorder Transfers Back date on Workorder
     odoo Workorder back-dating option Workorder back dating options Workorder Back date Operations Workorder
     odoo Back-date Options Workorder back dating options Workorder Back date Operations Back date Operations on Workorder
     odoo Workorder back date operations Workorder back date operation Workorder
     odoo Workorder back date odoo Workorder remarks transfer remarks Workorder delivery remarks Workorder remarks on Workorder
     odoo Workorder remarks odoo stock back date remarks on Workorder
     odoo Workorder stock accounting back date transfer back date on Workorder odoo

	""",
    'author' : 'BrowseInfo',
    'website': 'https://www.browseinfo.in',
    'currency': 'EUR',
    'price': 99, 
    'depends':['base','sale_management','stock','account','purchase','mrp','stock_account'],
     'data':['wizard/validate_internal.xml',
               'views/inherit_stock_picking.xml',
               'views/inherit_stock_picking_inventory.xml',
               'wizard/validate_internal_inventory.xml',
               'views/inherit_stock_picking_manufacturing.xml',
               'wizard/validate_internal_manufacturing.xml',
               'views/inherit_scrap_order_backdate.xml',
               'wizard/wizard_scrap_order.xml',
               'wizard/validate_delivery_wizard.xml'
               ],    
     'qweb': [],
    "auto_install": False,
    "installable": True,
    "live_test_url":'https://youtu.be/AZk-1QQCdpc',
    "images":['static/description/Banner.png'],
}
# vim:expandtab:smartindent:tabstop=4:softtabstop=4:shiftwidth=4:
