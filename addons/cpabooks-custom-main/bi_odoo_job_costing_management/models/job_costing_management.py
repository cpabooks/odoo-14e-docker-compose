...
class JobCostSheet(models.Model):
    ...
    @api.depends('profit_percent', 'total_material_cost', 'total_labour_cost', 'total_overhead_cost')
    def _compute_total_cost(self):
        for sheet in self:
            total = sheet.total_material_cost + sheet.total_labour_cost + sheet.total_overhead_cost
            if sheet.profit_percent != 0:
                sheet.total_cost = total + (total * sheet.profit_percent / 100)
            else:
