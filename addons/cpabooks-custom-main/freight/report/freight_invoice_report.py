...
class reportFreightInvoice(models.AbstractModel):
    ...
    def get_freight_main_charge_breakdown(self,ids,contSize,ContType):
        ...
        for rec in record.invoice_line_ids:
            if rec.product_id.charge_type.id==self.env.ref('freight.freight_charge_type_main').id:
    ...
    def get_freight_other_charge_breakdown(self,ids):
        ...
        record = self.env['account.move'].browse(ids)
        for rec in record.invoice_line_ids:
            if rec.product_id.charge_type.id==self.env.ref('freight.freight_charge_type_other').id:
                vals.append(rec)
        return vals
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            ...
            'mainChargeTypeId':self.env.ref('freight.freight_charge_type_main'),
            'otherChargeTypeId':self.env.ref('freight.freight_charge_type_other'),
