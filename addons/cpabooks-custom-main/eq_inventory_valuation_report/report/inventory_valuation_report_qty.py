...
class InventoryValuationReportqty(models.AbstractModel):
    _name = 'inventory.valuation.report.qty'
    ...
    def _get_beginning_inventory(self, record, product, warehouse, location=None):
        ...
        self._cr.execute(''' 
                        ...
                        FROM
                            ...
                            LEFT JOIN uom_uom pu ON (pt.uom_id=pu.id)
                            LEFT JOIN uom_uom u ON (m.product_uom=u.id)
                            WHERE m.date < %s AND (m.location_dest_id in %s) AND m.state='done' AND pp.active=True AND pp.id = %s AND m.picking_id is not null
                            GROUP BY  pp.id,pt.uom_id , m.product_uom ,pp.default_code,u.name,m.date
                            )
                            UNION ALL                            
                             (SELECT pp.id, pp.default_code,m.date,
                            ...
                            LEFT JOIN product_template pt ON (pp.product_tmpl_id=pt.id)
                            LEFT JOIN stock_location l ON (m.location_dest_id=l.id)    
                            LEFT JOIN mrp_production mp ON m.production_id=mp.id
                            LEFT JOIN uom_uom pu ON (pt.uom_id=pu.id)
                            LEFT JOIN uom_uom u ON (m.product_uom=u.id)
                            WHERE m.date < %s AND (m.location_dest_id in %s) AND m.state='done' AND pp.active=True AND pp.id = %s AND m.production_id is not null
                            GROUP BY  pp.id,pt.uom_id , m.product_uom ,pp.default_code,u.name,m.date
                            )
                            )
                        AS foo
                        GROUP BY id
                    ''', (from_date, tuple(locations), product_data, from_date, tuple(locations), product_data,from_date, tuple(locations), product_data))
    ...
    def get_product_sale_qty(self, record, warehouse, product=None, location=None):
        ...
        if product_data:
            ...
            get_production_module=self.env['ir.module.module'].sudo().search([('name','=','mrp')])
            if get_production_module:
                query='''SELECT pp.id AS product_id,pt.categ_id,sum(mp.product_qty) as production_qty FROM product_product pp
                            LEFT JOIN stock_move sm ON (sm.product_id = pp.id and sm.date >= '{}' and sm.date <= '{}' and sm.state = 'done' and sm.location_id != sm.location_dest_id)
                            LEFT JOIN stock_move_line smline ON (smline.product_id = pp.id and smline.state = 'done' and smline.location_id != smline.location_dest_id and smline.move_id = sm.id)
                            ...
                            LEFT JOIN uom_uom pu ON (pt.uom_id=pu.id)
                            LEFT JOIN uom_uom pu2 ON (smline.product_uom_id=pu2.id)
                            WHERE pp.id in {} AND smline.location_dest_id in {} and sm.production_id is not null
                            GROUP BY pt.categ_id, pp.id order by pt.categ_id
                '''.format(start_date, end_date, str(product_data).replace(',)',')'),str(tuple(locations)).replace(',)',')'))
                self._cr.execute(query=query)
                production_values = self._cr.dictfetchall()

            #endregion
            if production_values:
                for production in production_values:
                    for val in values:
                        if production['product_id']==val['product_id'] and production['categ_id']==val['categ_id']:
                            val['product_qty_in']=val['product_qty_in']+production['production_qty']
