...
class bulk_invoice(models.TransientModel):
    ...
    amount = fields.Float('Inv Amount')
    paid_amount = fields.Float('Pay Amount') 
    bulk_invoice_id = fields.Many2one('bulk.inv.payment')
    woff_account = fields.Many2one('account.account', 'Write-Off Account')
    woff_amount = fields.Float('Write-Off', default=0.0)
    due_amount = fields.Float('Balance Amount', default=0.0, compute='_compute_due_amount')

    @api.depends('amount', 'paid_amount', 'woff_amount')
    def _compute_due_amount(self):
        for rec in self:
            rec.due_amount = rec.amount - rec.paid_amount - rec.woff_amount
...
class bulk_inv_payment(models.TransientModel):
    ...
    get_inv_date = fields.Boolean('Get Invoice Date')
    ...
    @api.onchange('get_inv_date')
    @api.depends('get_inv_date', 'invoice_ids')
    def action_get_inv_date(self):
        self.ensure_one()
        if self.get_inv_date and self.invoice_ids:
            self.payment_date = self.invoice_ids[-1].invoice_id.invoice_date
        elif self.get_inv_date and not self.invoice_ids:
            self.get_inv_date = False
            raise ValidationError(_('No Invoice Added.'))
        else:
            self.get_inv_date = False
    ...
    def change_cheque_date(self):
        for rec in self:
            if rec.payment_date:
                rec.cheque_date = rec.payment_date

    
    def process_payment(self):
        for line in self.invoice_ids:
            if line.woff_amount != 0.0 and not line.woff_account:
                raise ValidationError(_(f"The Invoice '{line.invoice_id.name}' has no Write off Account!"))
        vals=[]
        for line in self.invoice_ids:
            if line.paid_amount > 0.0:
                vals.append({
                    ...
                    'woff_amount': line.woff_amount or False
                })
        ...
        for res in result:
            ...
            inv_ids = []
            total_woff_amt = 0
            woff_account = res.get('values')[0].get('woff_account', '')
            for inv_line in res.get('values'):
                invoice  = inv_line.get('invoice_id')
                woff_amt = inv_line.get('woff_amount', 0.0)
                ...
                line_list.append((0,0,{
                    ...
                    'woff_amount': woff_amt,
                    'woff_account': inv_line.get('woff_account', '') or False,
                }))
                paid_amt += inv_line.get('paid_amount')
                total_woff_amt += woff_amt
            payment_id.write({
                'adv_line_ids':line_list,
                'amount':paid_amt,
                'woff_amount': total_woff_amt,
                'woff_account': woff_account if woff_account else False
            })
