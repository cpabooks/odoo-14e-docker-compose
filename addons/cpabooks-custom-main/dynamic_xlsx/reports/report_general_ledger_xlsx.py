...
class InsGeneralLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, data, acc_lines, filter, workbook, sheet, sheet_2, record):
        format_header = workbook.add_format({
            'bold': True,
            'font_size': 10,
            'font': 'Arial',
        ...
        if filter.get('include_details', False):
            ...
            sheet.write_string(row_pos, 2, _('Partner'),
                                    format_header)
            # self.sheet.write_string(row_pos, 3, _('Ref'),
            #                         self.format_header)
        ...
        if acc_lines:
            for line in acc_lines:
                row_pos += 1
                sheet.merge_range(row_pos, 0, row_pos, 4, '            ' + acc_lines[line].get('code') + ' - ' + acc_lines[line].get('name'), line_header_left)
                ...
                if filter.get('include_details', False):
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            sheet.write_number(row_pos, 5, float(sub_line.get('debit')),
                                                    line_header_light_initial)
                            ...
                            sheet.write_string(row_pos, 2, sub_line.get('partner_name') or '',
                                                    line_header_light)
                            # self.sheet.write_string(row_pos, 3, sub_line.get('lref') or '',
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        if record:
            ...
            # Content section
            self.prepare_report_contents(data, account_lines, filters, workbook, sheet, sheet_2, record)
