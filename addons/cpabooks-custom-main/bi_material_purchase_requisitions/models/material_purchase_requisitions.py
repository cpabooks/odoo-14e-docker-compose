...
class MaterialPurchaseRequisition(models.Model):
    ...
    sequence = fields.Char(string='Name', copy=False, index=True, default=lambda self: _('New'))
    ...
    def create(self, vals):
        if vals.get('sequence', _('New')) == _('New'):
            vals['sequence'] = self.env['ir.sequence'].next_by_code('material.purchase.requisition') or _('New')
        result = super(MaterialPurchaseRequisition, self).create(vals)
        return result
    ...
    def confirm_requisition(self):
        for requisition in self:
            ...
            if template_id:
                values = email_template_obj.generate_email(requisition.id,
