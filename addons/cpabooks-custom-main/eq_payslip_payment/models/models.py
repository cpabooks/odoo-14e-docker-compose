...
class emp_payslip_payment_line(models.TransientModel):
    ...
    def generate_payment_move(self):
        ...
        if payslip_id.journal_id and not payslip_id.journal_id.default_credit_account_id:
            raise Warning(
                _('Payslip Salary Journal "%s" should be configured Credit Account.!') % (payslip_id.journal_id.name))
...
class hr_payslip(models.Model):
    ...
    def compute_pay_amount(self):
        for rec in self:
            ...
            already_paid_amount= sum(rec.payment_move_ids.line_ids.filtered(lambda l:l.account_id.user_type_id.type == 'payable' and l.move_id.state == 'posted').mapped('debit')) or 0.00
            rec.pay_amount = pay_amount-already_paid_amount
