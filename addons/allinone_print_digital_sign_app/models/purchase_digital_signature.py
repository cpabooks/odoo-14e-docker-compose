...
class PurchaseOrder(models.Model):
	...
	stamp = fields.Binary('Stamp', compute='_get_default_image')

	@api.depends('prepare_by','approve_by')
	def _get_default_image(self):
		for rec in self:
			if rec.prepare_by.user_stamp:
				rec.stamp = rec.prepare_by.user_stamp
			elif rec.approve_by.user_stamp:
				rec.stamp = rec.approve_by.user_stamp
			elif rec.prepare_by.user_stamp and rec.approve_by.user_stamp:
				rec.stamp = rec.prepare_by.user_stamp
