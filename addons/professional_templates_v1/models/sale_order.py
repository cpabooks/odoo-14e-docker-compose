...
class SO(models.Model):
    ...
    def onchange_partner_style(self):
        ...
        self.style = self.partner_id.style or self.env.user.company_id.df_style or \
                     self.env.ref('professional_templates_v1.df_style_for_all_reports').id
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        """
        Add 'date=None' parameter to match the updated signature
        """
        invoice_vals_prepare = super(SO, self)._create_invoices(grouped=grouped, final=final, date=date)
