...
class TemplateSettings(models.Model):
    ...
    can_access=fields.Boolean(compute="_get_can_access")

    @api.depends('paperformat_id')
    def _get_can_access(self):
        get_admin_power_group = self.env['res.groups'].sudo().search([('name', '=ilike', 'Admin Power')])
        if self.env.user.has_group('base.group_system') or (get_admin_power_group and self.env.user.id in get_admin_power_group.users.ids):
    ...
    def _default_so_template(self):
        def_tpl = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.SO\_%\_document'),
             ('type', '=', 'qweb'),('name','=','Quotation Common Template')],
            order='key asc',
            limit=1)
    ...
    def _default_po_template(self):
        def_tpl = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.PO\_%\_document'),
             ('type', '=', 'qweb'),('name','=','TVA Template')],
            order='key asc',
            limit=1)
    ...
    def _default_rfq_template(self):
        def_tpl = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.RFQ\_%\_document'),
             ('type', '=', 'qweb'),('name','=','TVA Template')],
            order='key asc',
            limit=1)
        return def_tpl or self.env.ref(
    ...
    def _default_dn_template(self):
        def_tpl = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.DN\_%\_document'),
             ('type', '=', 'qweb'),('name','=','100 miles (TVA) Template')],
            order='key asc',
            limit=1)
    ...
    def _default_pk_template(self):
        def_tpl = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.PICK\_%\_document'),
             ('type', '=', 'qweb'),('name','=','100 miles (TVA) Template')],
            order='key asc',
            limit=1)
    ...
    def _default_inv_template(self):
        def_tpl = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.INVOICE\_%\_document'),
             ('type', '=', 'qweb'),('name','=','100 Miles Template')],
            order='key asc',
            limit=1)
        return def_tpl or self.env.ref(
            'account.report_invoice_document_with_payments')

    @api.onchange('template_inv','template_so','template_po','template_rfq','template_dn','template_pk')
    def reset_template(self):
        self.reset_template_bool = True

    def reset_default(self):
        self.reset_template_bool = False

        get_so_template = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.SO\_%\_document'),
             ('type', '=', 'qweb'), ('name', '=', 'Quotation Common Template')],
            order='key asc',
            limit=1)
        get_dn_template = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.DN\_%\_document'),
             ('type', '=', 'qweb'), ('name', '=', '100 miles (TVA) Template')],
            order='key asc',
            limit=1)
        get_pk_template = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.PICK\_%\_document'),
             ('type', '=', 'qweb'), ('name', '=', '100 miles (TVA) Template')],
            order='key asc',
            limit=1)
        get_inv_template = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.INVOICE\_%\_document'),
             ('type', '=', 'qweb'), ('name', '=', 'Invoice Common Template')],
            order='key asc',
            limit=1)
        get_po_template = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.PO\_%\_document'),
             ('type', '=', 'qweb'), ('name', '=', 'Common_LPO')],
            order='key asc',
            limit=1)
        get_rfq_template = self.env['ir.ui.view'].search(
            [('key', 'like', 'professional_templates_v1.RFQ\_%\_document'),
             ('type', '=', 'qweb'), ('name', '=', 'Odoo Template')],
            order='key asc',
            limit=1)
        self.template_so = get_so_template.id
        self.template_inv = get_inv_template.id
        self.template_pk = get_pk_template.id
        self.template_dn = get_dn_template.id
        self.template_po = get_po_template.id
        self.template_rfq = get_rfq_template.id




    is_item_code_visible = fields.Boolean(string="Show Item Code", default=False)
    is_customer_bold = fields.Boolean(string="Partner Name Bold")
    is_customer_upper = fields.Boolean(string="Partner Name Capital Format")
    # is_customer_icon_remove = fields.Boolean(string="Partner Icon Remove")
    # is_customer_add_icon_remove = fields.Boolean(string="Partner Address Icon Remove")
    is_icon_hide=fields.Boolean(string="Hide Icon", help="Icon remove of customer,serial,item & description,class,qty")
    reset_template_bool = fields.Boolean('RESET')
    ...
    template_inv = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'professional_templates_v1.INVOICE\_%\_document' )]",
        required=False)
    template_so = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'professional_templates_v1.SO\_%\_document' )]",
        required=False)
    ...
    template_po = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'professional_templates_v1.PO\_%\_document' )]",
        required=False)
    ...
    template_rfq = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'professional_templates_v1.RFQ\_%\_document' )]",
        required=False)
    ...
    template_dn = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'professional_templates_v1.DN\_%\_document' )]",
        required=False)
    ...
    template_pk = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'professional_templates_v1.PICK\_%\_document' )]",
        required=False)
...
''',
    ...
    wm_color = fields.Char('Watermark color (Hex)',
                           size=7,
                           default="#5A5B5C",
                           required=True,
