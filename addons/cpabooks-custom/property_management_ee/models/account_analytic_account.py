...
class AccountAnalyticAccount(models.Model):
        ...
        for tenancy_rec in self:
            if tenancy_rec.state not in ('draft', 'cancelled'):
                state_description_values = {
                    elem[0]: elem[1] for elem in self._fields[
    ...
    @api.depends('invc_id.payment_state')
    def compute_amount_return(self):
        ...
        for data in self:
            if data.invc_id.payment_state == 'paid':
               # and data.invc_id.amount_residual == 0.00
    ...
    def button_return(self):
        ...
        for rec in self:
            inv_line_values = {
                ...
                'quantity': 1,
                'account_id': rec.property_id.account_depreciation_expense_id.id or False,
    ...
    def button_start(self):
        ...
        for current_rec in self:
            if current_rec.is_property == True:
    ...
    def cron_property_tenancy(self):
        ...
        model_data_id = self.env.ref(
            'property_management_ee.property_email_template')
