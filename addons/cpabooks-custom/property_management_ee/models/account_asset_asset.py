...
class AccountAssetAsset(models.Model):
    ...
    custom_asset_name = fields.Char(string="Asset Name")

    @api.depends('custom_asset_name')
    def _compute_name(self):
        for record in self:
            record.name = record.custom_asset_name
    ...
    def _compute_occupancy_rates(self):
        ...
        for prop_rec in self:
            diffrnc = 0
            occ_rate = 0
            if prop_rec.acquisition_date:
                pur_diff = datetime.now().date() - prop_rec.acquisition_date
    ...
    # @api.depends('original_value','salvage_value','depreciation_move_ids')
    ...
    #             total_amount = sum(
    #                 line.amount for line in rec.depreciation_move_ids
    #                 if line.move_check)
    #             total_residual = \
    #                 rec.original_value - total_amount - rec.salvage_value
    ...
    def create(self, vals):
        ...
        # Need to fix this
        if 'custom_asset_name' in vals.keys():
            vals['name'] = vals['custom_asset_name']
            acc_analytic_id.create({'name':vals['custom_asset_name'],'state':'draft'})
            vals['name'] = vals['custom_asset_name']
        if 'custom_asset_name' in self.env.context.keys():
            acc_analytic_id.create({'name': self.env.context.get('custom_asset_name')})
            vals['name'] = self.env.context.get('custom_asset_name')
            vals['custom_asset_name'] = self.env.context.get('custom_asset_name')
        # Need to fix this
    ...
    def unit_price_calc(self):
        ...
        if self.unit_price and self.gfa_feet:
            ...
            #TODO: If needed to match with any other field then, we can do this later
