...
class TemplateSettings(models.Model):
    ...
    @api.model
    def set_a4(self):
        # get_a4=self.env['report.paperformat'].sudo().search([('name','=','A4')])
        # for data in get_a4:
        #     data.margin_top=40
        #     data.margin_bottom=35
        #     data.margin_left=7
        #     data.margin_right=7
        #     data.header_spacing=40
        #     data.dpi=97
        ...
        get_rfq_template=self.env['ir.ui.view'].search(
            ...
            order='key asc',
            limit=1)
        get_default_template_setup=self.env['report.template.settings'].sudo().search([('name','=','Default style')])
        for rec in get_default_template_setup:
            rec.template_so=get_so_template.id
            rec.template_dn=get_dn_template.id
            rec.template_pk=get_pk_template.id
            rec.template_inv=get_inv_template.id
            rec.template_po=get_po_template.id
            rec.template_rfq=get_rfq_template.id
            rec.header_font='14'
            rec.body_font='14'
            rec.footer_font='12'
            rec.watermark="watermark = False"

            # rec.name_color="#093671"
            # rec.cust_color=""
    ...
    template_so = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'professional_templates.SO\_%\_document' )]",
        required=False)
    ...
    template_dn = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'professional_templates.DN\_%\_document' )]",
        required=False)
    ...
    template_pk = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'professional_templates.PICK\_%\_document' )]",
        required=False)
