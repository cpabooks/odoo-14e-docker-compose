        ...
        for order in self:
            ...
            if order.discount_type == 'global':
                ...
                for line in order.order_line:
                    ...
                    # taxes = get_exclusive_five_per_tax.compute_all(line_wise_discount, quantity=1, product=None, partner=order.partner_shipping_id)['taxes']
                    for tax in line.tax_id:
                        group = tax.tax_group_id
                        res.setdefault(group, {'amount': 0.0, 'base': 0.0})
                        for t in taxes:
                ...
                if res_config.tax_discount_policy == 'tax':
                    if order.discount_type == 'line':
                        ...
                        order.update({
                            ...
                            'total_after_discount': amount_untaxed - line_discount if line_discount!=0 else 0,
                            # 'total_after_discount': amount_untaxed + amount_tax - line_discount,
                        })
                        ...
                        if order.discount_method == 'per':
                            ...
                            order.update({
                                ...
                                'total_after_discount': amount_untaxed - order_discount if order_discount!=0 else 0,
                                # 'total_after_discount': amount_untaxed + amount_tax - order_discount,
                            })
                            ...
                            order.update({
                                ...
                                # 'total_after_discount':  amount_untaxed + amount_tax ,
                            })
                    else:
                        order.update({
                            'amount_untaxed': amount_untaxed,
                    ...
                    if order.discount_type == 'line':
                        ...
                        order.update({
                            ...
                            'total_after_discount': amount_untaxed - applied_discount if applied_discount!=0 else 0,
                            # 'total_after_discount': amount_untaxed + amount_tax - applied_discount,
                        })
                    elif order.discount_type == 'global':
                        # order.discount_amt_line = 0.00
                        # if order.discount_method == 'fix':
                        ...
                        #         'discount_amt': order_discount,
                        #         'total_after_discount': amount_untaxed - order_discount if order_discount!=0 else 0,
                        #         # 'total_after_discount': amount_untaxed + sums - order_discount,
                        #     })
                        # elif order.discount_method == 'per':
                        #     order_discount = amount_untaxed * (order.discount_amount / 100)
                        #     price_reduce = amount_untaxed - order_discount
                                ...
                                for line in order.order_line:
                                    if line.tax_id:
                                        ...
                                        taxes = line.tax_id.compute_all(discount, \
                                                            ...
                                                            partner=order.partner_id)
                                        #
                                        # taxes = get_exclusive_five_per_tax.compute_all(discount, \
                                        #                                 order.currency_id, 1.0, product=line.product_id, \
                                        #                                 partner=order.partner_id)
                                        sums += sum(t.get('amount', 0.0) for t in taxes.get('taxes', []))
                            order.update({
                                ...
                                for line in order.order_line:
                                    if line.tax_id:
                                        ...
                                        taxes = line.tax_id.compute_all(round(discount,2), \
                                                            order.currency_id,1.0, product=line.product_id, \
                                                            partner=order.partner_id)
                                        # taxes = get_exclusive_five_per_tax.compute_all(round(discount,2), \
                                        #                     order.currency_id,1.0, product=line.product_id, \
                                        #                     partner=order.partner_id)
                                        sums += sum(t.get('amount', 0.0) for t in taxes.get('taxes', []))
                            order.update({
