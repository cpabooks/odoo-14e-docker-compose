...
class report_account_general_ledger_bi(models.AbstractModel):
    ...
    def _do_query_unaffected_earnings(self, options, line_id, company=None):
        ...
        tables, where_clause, where_params = self.env['account.move.line'].with_context(self._set_context(options))._query_get(domain=aml_domain)
    ...
    def _do_query(self, options, line_id, group_by_account=True, limit=False):
        ...
        tables, where_clause, where_params = self.env['account.move.line'].with_context(self._set_context(options))._query_get()
        custom_accounts = []
    ...
    def _group_by_account_id(self, options, line_id):
        ...
        for account_id, result in results.items():
            ...
            if account.user_type_id == unaffected_earnings_type and account.company_id not in unaff_earnings_treated_companies:
                ...
                unaffected_earnings_results = unaffected_earnings_per_company[account.company_id.id]
    ...
    def _get_taxes(self, options, journal):
        tables, where_clause, where_params = self.env['account.move.line'].with_context(self._set_context(options))._query_get()
        query = """
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for account in sorted_accounts:
            ...
            if unfold_all:
                ...
                for line in amls:
                    ...
                    if line.payment_id:
                        communication = line.payment_id.payment_reference
        ...
        if len(journals) == 1 and journals[0].get('type') in ['sale', 'purchase'] and not line_id:
            ...
            taxess = self._get_taxes(options, journals[0])
            if len(taxess.keys()):
                for tax, values in taxess.items():
                    lines.append({
                        'id': '%s_tax' % (tax.id,),
