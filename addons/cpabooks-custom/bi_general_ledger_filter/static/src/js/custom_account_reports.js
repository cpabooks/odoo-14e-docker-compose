odoo.define('custom_account_accounting_report.account_report_generic', function (require) {
'use strict';
...
var _t = core._t;
    ...
    var M2MCustomAccountFilters = Widget.extend(StandaloneFieldManagerMixin, {
        /**
         * @constructor
         * @param {Object} fields
         */
    ...
    accountReportsWidget.include({
        ...
        custom_events: _.extend({}, accountReportsWidget.prototype.custom_events, {
            ...
            'value_changed': function(ev) {
                var self = this;
                self.report_options.custom_account_ids = ev.data.custom_account;
                self.report_options.custom_account_type_ids = ev.data.custom_account_type;
                ...
                return self.reload().then(function () {
                    ...
                    self.$searchview_buttons.find('.account_custom_account_filter').click();
                });
             },
        }),
        ...
        render_searchview_buttons: function() {
            ...
            if (this.report_options.custom_account) {
                
                if (!this.M2MCustomAccountFilters) {
                    var fields = {};
                    if ('custom_account_ids' in this.report_options) {
                        fields['custom_account'] = {
                            label: _t('Account'),
                            modelName: 'account.account',
                            value: this.report_options.custom_account_ids.map(Number),
                        };
                    }

                    if ('custom_account_type_ids' in this.report_options) {
                        fields['custom_account_type'] = {
                            label: _t('Account Type'),
                            modelName: 'account.account.type',
                            value: this.report_options.custom_account_type_ids.map(Number),
                        };
                    }

                    if (!_.isEmpty(fields)) {
                        this.M2MCustomAccountFilters = new M2MCustomAccountFilters(this, fields);
                        this.M2MCustomAccountFilters.appendTo(this.$searchview_buttons.find('.js_account_custom_account_m2m'));
                    }
                } else {
                    this.$searchview_buttons.find('.js_account_custom_account_m2m').append(this.M2MCustomAccountFilters.$el);
                }
            }
