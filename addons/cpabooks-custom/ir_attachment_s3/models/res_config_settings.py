...
class S3Settings(models.TransientModel):
    ...
    s3_condition = fields.Char(
        ...
        help="""Specify valid odoo search domain here,
                               ...
                               Empty condition means all models""", required=True
    )
    ...
    def get_values(self):
        ...
        #
        if ICPSudo.get_param("s3.condition"):
            s3_condition = ICPSudo.get_param("s3.condition")
        else:
            s3_condition = ICPSudo.get_param("s3.condition",
                                             default="[('res_model', 'in', ['crm.lead', 'sale.order', 'pdc.wizard', 'purchase.order', 'hr.payslip', 'documents.document', 'account.move', 'project.project', 'hr.employee', 'product.template', 'dev.employee.document', 'daily.site.report'])]") #[('res_model', 'in', ['sale.order','account.move','documents.document','purchase.order'])]
    ...
    def set_values(self):
        ...
        # ICPSudo.set_param("s3.condition", self.s3_condition or "[('res_model', 'in', ['sale.order','account.move','documents.document','purchase.order'])]")
        ICPSudo.set_param("s3.condition", self.s3_condition)
