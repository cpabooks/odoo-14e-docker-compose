    ...
    $('.address-column').on('click', '.js_add_address', _.debounce( 
        function (ev) {
            ...
            if (valid) {
                disableButtons($(ev.currentTarget));
                var parent_id = $billing_form.find('input[name="partner_id"]').val();
                ajax.post('/shop/check_address?' +$add_form.serialize()+'&parent_id='+parent_id+'&xhr=1').then (data => {
                    let json = {};
                    try { json = JSON.parse(data); } catch (error) { console.error(error); };
                    if (!json.error) { return render_kanban(csrf_token, partner_id); };
                    for (const field in json.error) {
                        $shipping_form.find(`[name="${field}"]`).closest('.form-group').addClass('.has-error');
                    };
                    new Dialog(this, {
                        title: 'Error',
