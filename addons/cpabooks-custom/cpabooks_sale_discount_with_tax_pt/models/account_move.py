...
class account_move(models.Model):
    ...
    def _compute_amount(self):
        for move in self:
            ...
            move.amount_total_signed = sign * (total_currency if len(currencies) == 1 else total)
            ...
            move.amount_total_signed = move.amount_untaxed - res + move.amount_tax
            ...
            for rec in self:
                if res_config.tax_discount_policy == 'tax':
                    if rec.discount_type == 'line':
                        ...
                        rec.amount_total_signed = rec.amount_tax + rec.amount_untaxed - rec.discount_amt_line
                        ...
                        if rec.discount_method == 'fix':
                            ...
                            rec.amount_total_signed = rec.amount_tax + rec.amount_untaxed - rec.discount_amt
                            ...
                            rec.amount_total_signed = rec.amount_tax + rec.amount_untaxed
                    ...
                    if rec.discount_type == 'line':
                        ...
                        if rec.discount_method == 'fix':
                            ...
                            rec.amount_total_signed = sums + rec.amount_untaxed - rec.discount_amt
                ...
                if rec.move_type=='out_invoice':
                    if rec.total_after_discount>0:
                        rec.amount_untaxed_signed=rec.total_after_discount
                if rec.move_type=='out_refund':
                    if rec.total_after_discount>0:
                        rec.amount_untaxed_signed=(-1)*rec.total_after_discount
                        rec.amount_total_signed=(-1)*rec.amount_total_signed
                        rec.amount_residual=(-1)*rec.amount_residual
    ...
    def write(self,vals):
        for parent in self:
            if parent.move_type=='out_invoice':
                name_flag = False;
                for line in parent.line_ids:
                    if line.name == "Discount":
                        name_flag = True;
                if parent.discount_type == 'line':
                    if 'discount_amt_line' in vals.keys():
                        price = vals['discount_amt_line']
                    else:
                        price = parent.discount_amt_line
                elif parent.discount_type == 'global':
                    if 'discount_amt' in vals.keys():
                        price = vals['discount_amt']

                    else:
                        price = parent._calculate_discount()
                else:
                    price = 0
                if not name_flag:
                    if parent.discount_account_id:
                        discount_vals = {
                            'account_id': parent.discount_account_id,
                            'quantity': 1,
                            'price_unit': -price,
                            'name': "Discount",
                            'exclude_from_invoice_tab': True,
                        ...
                        if 'invoice_line_ids' in vals.keys():
                            discount_existency=0
                            for line in vals['invoice_line_ids']:
                                if line[0]==0 and 'name' in line[2].keys() and line[2]['name']=='Discount':
                                    discount_existency=1
                            if discount_existency==0:
                                vals['invoice_line_ids'].append((0, 0, discount_vals))

                        else:
                            vals['invoice_line_ids']=[(0, 0, discount_vals)]
                ...
                for rec in parent.line_ids:
                    if parent.discount_type != 'line':
                        if rec.name == "Discount":
                            rec.with_context(check_move_validity=False).write({'price_unit':-parent._calculate_discount()})
                            if 'discount_amount' in vals.keys() and vals['discount_amount']==0:
                                if 'line_ids' in vals.keys():
                                    vals['line_ids'].append((2,rec.id))
                                else:
                                    vals['line_ids'] = [(2,rec.id)]
                        if rec.name == parent.name:
