...
class HrSalaryRules(models.Model):
    ...
    def action_update_salary_rule_debit_credit(self):
        get_payroll_stucture_type=self.env['hr.payroll.structure.type'].search([('name','ilike','Hourly Pay on Timesheet%')])
        for hpot in get_payroll_stucture_type:
            hpot.wage_type='hourly'
